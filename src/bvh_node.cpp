...
Bvh_node::Bvh_node(const Hittable_list& obj_list): Bvh_node(obj_list.get_objects(), 0, obj_list.size()) {}
Bvh_node::Bvh_node(const std::vector<std::shared_ptr<hittable>>& obj_ptrs, int start, int end) {
...
bool Bvh_node::hit(Ray ray_in,  Interval2f intv, HitRecord& rec_) const {
